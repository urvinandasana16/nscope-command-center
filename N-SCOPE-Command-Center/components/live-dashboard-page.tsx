...
import { Activity, AlertTriangle, Building2, Clock, Laptop, MapPin, RefreshCw, ShieldAlert, Ticket, Users } from "lucide-react";
...
export function LiveDashboardPage() {
  ...
  useEffect(() => {
    try {
      const user = JSON.parse(window.localStorage.getItem("nscope_user") ?? "null") as { role?: string } | null;
  ...
  return (
    <>
      ...
      {currentRole === "SUPER_ADMIN" && (
        <Card className="mt-4">
          <div className="mb-3 flex items-center justify-between">
            <h2 className="text-base font-semibold">Engineer Workload</h2>
            <Users className="h-4 w-4 text-slate-400" />
          </div>
          <DataTable
            columns={["Engineer", "Role", "Assigned Clients", "Assigned Sites", "Open Tickets"]}
            emptyMessage="No active engineers found. Create an engineer from Admin Users."
            rows={(summary.engineer_workload ?? []).map((engineer) => [
              <div key="engineer"><p className="font-medium">{engineer.name}</p><p className="text-xs text-slate-500">{engineer.email}</p></div>,
              engineer.role.replace(/_/g, " "),
              String(engineer.assigned_clients),
              String(engineer.assigned_sites),
              String(engineer.open_tickets),
            ])}
          />
        </Card>
      )}
