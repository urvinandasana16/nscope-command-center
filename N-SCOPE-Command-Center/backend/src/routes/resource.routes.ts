...
apiRoutes.get("/users/engineers", async (_req, res) => {
  const engineers = await prisma.user.findMany({
    where: {
      status: "ACTIVE",
      role: { in: ["SUPER_ADMIN", "NOC_ENGINEER", "SUPPORT_ENGINEER"] },
    },
...
apiRoutes.use(
  ...
  crudRoutes(
    createCrudController({
      ...
      include: { assignedEngineer: { select: { id: true, name: true, email: true, role: true } } },
    }),
    adminOnly,
    adminOnly
  )
...
apiRoutes.use(
  ...
  crudRoutes(
    createCrudController({
      ...
      scopedByClient: true,
      include: {
        client: { include: { assignedEngineer: { select: { id: true, name: true, email: true, role: true } } } },
        site: { include: { assignedEngineer: { select: { id: true, name: true, email: true, role: true } } } },
        agent: true,
