...
import { createSite, deleteSite, listSites, updateSite } from "@/services/siteService";
...
import { createUser, listUsers, type UserRecord } from "@/services/userService";
...
export function LiveClientsPage() {
  ...
  async function load() {
    try {
      const [data, userData] = await Promise.all([listClients(), listUsers()]);
  ...
  return (
    <CrudFrame
      ...
      onRefresh={load}
    >
      {editing && <ClientForm value={editing} onCancel={() => setEditing(null)} onSave={save} />}
      ...
      <DataTable
        columns={["Client Name", "Contact Person", "Email", "Phone", "SLA Plan", "Status", "Actions"]}
        rows={rows.map((row) => [
          row.name,
          row.contactPerson,
          row.email,
...
export function LiveSitesPage() {
  ...
  async function load() {
    try {
      const [clientData, siteData] = await Promise.all([listClients(), listSites()]);
  ...
  return (
    <CrudFrame title="Sites" description="Manage client locations and network ranges." status={status} actionLabel="Add Site" onAdd={() => setEditing(blankSite(clients))} onRefresh={load}>
      {editing && <SiteForm value={editing} clients={clients} onCancel={() => setEditing(null)} onSave={save} />}
      <DataTable
        columns={["Site Name", "Client", "Location", "Network Range", "Status", "Actions"]}
        rows={rows.map((row) => [row.name, row.clientName, row.location, row.networkRange, <Badge key="status">{displayStatus(row.status)}</Badge>, <RowActions key="actions" onEdit={() => setEditing(row)} onDelete={() => remove(row)} />])}
      />
    </CrudFrame>
  );
}
...
export function LiveDevicesPage() {
  ...
  async function load() {
    try {
      const [clientData, siteData, deviceData, remoteData] = await Promise.all([
        listClients(),
        listSites(),
        listDevices(),
  ...
  return (
    <CrudFrame title="Devices" description="Compact endpoint inventory with remote actions, agent state, and client/site filtering." status={status} actionLabel="Add Device" onAdd={() => setEditing(blankDevice(clients, sites))} onRefresh={load}>
      ...
      <Card className="mb-4 p-3">
        <div className="grid gap-3 md:grid-cols-[minmax(180px,1fr)_180px_180px_150px]">
...
export function LiveTicketsPage() {
  ...
  return (
    <CrudFrame title="Tickets" description="Create tickets, link them with devices, update status, and add comments." status={status} actionLabel="Create Ticket" onAdd={() => setEditing(blankTicket(clients, sites, devices, assets))} onRefresh={load}>
      {editing && <TicketForm value={editing} clients={clients} sites={sites} devices={devices} assets={assets} onCancel={() => setEditing(null)} onSave={save} />}
      {commentTicket && (
        <Card className="mb-5">
      ...
      <DataTable
        columns={["Ticket ID", "Client", "Device", "Asset", "Subject", "Priority", "Status", "Comments", "SLA Due", "Actions"]}
...
function ClientForm({ value, onCancel, onSave }: { value: ClientRecord; onCancel: () => void; onSave: (value: ClientRecord) => void }) {
...
function SiteForm({ value, clients, onCancel, onSave }: { value: SiteRecord; clients: ClientRecord[]; onCancel: () => void; onSave: (value: SiteRecord) => void }) {
  const [form, setForm] = useState(value);
  return (
...
function TicketForm({ value, clients, sites, devices, assets, onCancel, onSave }: { value: TicketRecord; clients: ClientRecord[]; sites: SiteRecord[]; devices: DeviceRecord[]; assets: AssetRecord[]; onCancel: () => void; onSave: (value: TicketRecord) => void }) {
  const [form, setForm] = useState(value);
  const filteredSites = sites.filter((site) => site.clientId === form.clientId);
  const filteredDevices = devices.filter((device) => device.clientId === form.clientId);
...
function toSitePayload(row: SiteRecord) {
  return { clientId: row.clientId, name: row.name, location: row.location, networkRange: row.networkRange, status: row.status };
}
...
function toTicketPayload(row: TicketRecord) {
  return { ticketNumber: row.ticketNumber, clientId: row.clientId, siteId: row.siteId || null, deviceId: row.deviceId || null, assetId: row.assetId || null, subject: row.subject, description: row.description, priority: row.priority, status: row.status, slaDue: toIso(row.slaDue) };
}

function blankClient(): ClientRecord {
  return { id: "", name: "", contactPerson: "", email: "", phone: "", address: "", status: "HEALTHY", slaPlan: "Gold 24x7" };
}

function blankSite(clients: ClientRecord[]): SiteRecord {
  return { id: "", clientId: clients[0]?.id ?? "", clientName: clients[0]?.name ?? "", name: "", location: "", networkRange: "", status: "HEALTHY" };
}
...
function blankTicket(clients: ClientRecord[], sites: SiteRecord[], devices: DeviceRecord[], assets: AssetRecord[] = []): TicketRecord {
  ...
  return { id: "", ticketNumber: `NSC-${Date.now().toString().slice(-4)}`, clientId, clientName: clients[0]?.name ?? "", siteId: device?.siteId ?? asset?.siteId ?? sites.find((site) => site.clientId === clientId)?.id, deviceId: device?.id, deviceName: device?.hostname, assetId: asset?.id, assetName: asset?.name ?? asset?.assetTag, subject: "", description: "", priority: "MEDIUM", status: "OPEN", comments: [] };
}
